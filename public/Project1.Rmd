...
summary_stats_decade <- billboard_music %>% group_by(decade) %>%
  summarize("mean danceability"=mean(danceability), sd_danceability=sd(danceability), mean_energy=mean(energy), sd_energy=sd(energy)) %>% round(2)

kable(head(summary_stats_decade))

summary_stats_decade2 <- billboard_music %>% group_by(decade) %>%
  summarize(mean_acousticness=mean(acousticness), sd_acousticness=sd(acousticness), mean_valence=mean(valence), sd_valence=sd(valence)) %>% 
  round(2)
